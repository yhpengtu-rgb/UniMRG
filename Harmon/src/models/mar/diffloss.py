...
class DiffLoss(nn.Module):
    ...
    def sample(self, z, temperature=1.0, cfg=1.0):
        ...
        device = z.device
        dtype = self.net.input_proj.weight.dtype

        if not cfg == 1.0:
            noise = torch.randn(z.shape[0] // 2, self.in_channels, device=device, dtype=dtype)
            noise = torch.cat([noise, noise], dim=0)
            model_kwargs = dict(c=z, cfg_scale=cfg)
            sample_fn = self.net.forward_with_cfg
        else:
            noise = torch.randn(z.shape[0], self.in_channels, device=device, dtype=dtype)
            model_kwargs = dict(c=z)
...
class SimpleMLPAdaLN(nn.Module):
    ...
    def forward(self, x, t, c):
        ...
        use_checkpoint = self.grad_checkpointing and torch.is_grad_enabled() and (x.requires_grad or y.requires_grad)
